...
export default function AdminSubscriptions() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const [plansResponse, promoResponse] = await Promise.all([
          apiFetch(`${apiBase}/admin/subscriptions`),
          apiFetch(`${apiBase}/admin/promo-codes`),
        ]);

        if (!plansResponse.ok || !promoResponse.ok) {
          throw new Error("Не удалось загрузить тарифы и промокоды");
        }
        ...
        setPlans([]);
        setPromoCodes([]);
        setError(err instanceof Error ? err.message : "Не удалось загрузить тарифы и промокоды");
      } finally {
        setLoading(false);
      }
    };
  ...
  const savePlan = async () => {
    ...
    try {
      const res = await apiFetch(`${apiBase}/admin/subscriptions/${editingPlan}`, {
        method: "PUT",
        headers: jsonHeaders(),
      ...
      if (!res.ok) {
        throw new Error("Не удалось сохранить тариф");
      }
      ...
      toast({
        variant: "destructive",
        title: t("error"),
        description: err instanceof Error ? err.message : "Не удалось сохранить тариф",
      });
    }
  };
  ...
  const addPromo = async () => {
    ...
    try {
      const res = await apiFetch(`${apiBase}/admin/promo-codes`, {
        method: "POST",
        headers: jsonHeaders(),
      ...
      if (!res.ok) {
        throw new Error("Не удалось добавить промокод");
      }
      ...
      toast({
        variant: "destructive",
        title: t("error"),
        description: err instanceof Error ? err.message : "Не удалось добавить промокод",
      });
    }
  };
  ...
  const deletePromo = async (id: string) => {
    try {
      const res = await apiFetch(`${apiBase}/admin/promo-codes/${id}`, { method: "DELETE" });
      if (!res.ok) {
      ...
      toast({
        variant: "destructive",
        title: t("error"),
        description: err instanceof Error ? err.message : "Не удалось удалить промокод",
      });
    }
  };
  ...
  const togglePromo = async (promo: PromoCode) => {
    try {
      const res = await apiFetch(`${apiBase}/admin/promo-codes/${promo.id}`, {
        method: "PUT",
        headers: jsonHeaders(),
      ...
      if (!res.ok) {
        throw new Error("Не удалось обновить промокод");
      }
      ...
      toast({
        variant: "destructive",
        title: t("error"),
        description: err instanceof Error ? err.message : "Не удалось обновить промокод",
      });
    }
  };
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {loading && plans.length === 0 && (
          <div className="md:col-span-3 glass-panel rounded-3xl p-8 text-center text-sm text-muted-foreground">
            Загрузка тарифов...
          </div>
        )}
        {plans.map((plan) => {
            ...
            <div key={plan.id} className="glass-panel p-6 rounded-3xl relative flex flex-col">
              ...
              {isEditing ? (
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setEditData({ ...editData, discountPercent: parseInt(e.target.value, 10) || 0 })}
                        className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground"
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        type="number"
                        value={editData.tokensPerMonth ?? 0}
                        onChange={(e) => setEditData({ ...editData, tokensPerMonth: parseInt(e.target.value, 10) || 0 })}
                        className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="number"
                        value={editData.agentsLimit ?? 0}
                        onChange={(e) => setEditData({ ...editData, agentsLimit: parseInt(e.target.value, 10) || 0 })}
                        className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground"
                      />
                    </div>
                  </div>
                ...
                <>
                  ...
                  <button onClick={() => startEdit(plan)} className={`w-full py-3 rounded-xl font-semibold transition-all ${plan.planId === "pro" ? "bg-primary text-white shadow-lg shadow-primary/20 hover:bg-primary/90" : "bg-muted text-foreground hover:bg-muted/80"}`}>
                    {t("editPlan")}
                  </button>
                </>
              )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewPromo({ ...newPromo, discountPercent: parseInt(e.target.value, 10) || 0 })}
                  className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground mt-0.5"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewPromo({ ...newPromo, usageLimit: parseInt(e.target.value, 10) || 0 })}
                  className="w-full px-2 py-1.5 rounded-lg border border-border bg-background text-sm text-foreground mt-0.5"
                />
              </div>
              <div>
            ...
            <div key={promo.id} className="glass-panel px-4 py-3 rounded-xl flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`px-2 py-1 rounded-lg text-xs font-medium transition-colors ${promo.isActive ? "bg-green-500/10 text-green-400 hover:bg-green-500/20" : "bg-red-500/10 text-red-400 hover:bg-red-500/20"}`}
                >
